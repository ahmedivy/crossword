...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        for v in self.domains:
            ...
            for x in self.domains[v]:
                if len(x) != v.length:
                    inconsistentWords.append(x)
            for word in inconsistentWords:
    ...
    def revise(self, x, y):
        ...
        for X in self.domains[x]:
            if X[i] not in  [Y[j] for Y in self.domains[y]]:
                toRemove.add(X)
                revised = True
    ...
    def consistent(self, assignment):
        ...
        for v in assignment.keys():
            if assignment[v] in [value for value in list(assignment.values()) if value != assignment[v]]:
                return False
    ...
    def order_domain_values(self, var, assignment):
        ...
        # With Heuristic
        # Create dict to keep track of changes
        toTrack = {word: 0 for word in self.domains[var]}
        ...
        return sorted([x for x in toTrack], key = lambda x: toTrack[x])
    ...
    def select_unassigned_variable(self, assignment):
        ...
        # If there is a tie in domain
        if len(unAssigned) not in [0, 1, 2]:
            if unAssigned[0] == unAssigned[1]:
                unAssigned.sort(key = lambda x: len(self.crossword.neighbors(x)), reverse=True)
                return unAssigned[0]
        return unAssigned[0]
